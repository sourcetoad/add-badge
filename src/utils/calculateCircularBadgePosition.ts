...
export default function calculateCircularBadgePosition(
  ...
  const { angle, rotation, rotatedWidth, rotatedHeight } = getRotatedBadgeInfo(badge, gravity);

  // Calculate the distance from the center of the container to the bottom-center of
  // the badge.
  const distance = Math.sqrt(Math.max(0, Math.pow(circleRadius, 2) - Math.pow(badge.width / 2, 2)));

  const trigAngle = ((angle - 90) * Math.PI) / 180;

  const x =
    container.width / 2 - (distance - badge.height / 2) * Math.cos(trigAngle) + -rotatedWidth / 2;
